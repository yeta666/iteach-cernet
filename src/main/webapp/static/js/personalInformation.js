...
function showUserInfo(backInfo) {
	...
	if (null != data) {
		...
		for ( var i = 0; i < 7; i++) {
			...
			if(arr[i]==undefined)
				arr[i]="";
			$(id).html("<input id='"+i+"' type='text' class='form-control' readonly='readonly' value='"+arr[i]+"'><span "+(i<3?"class='no-info'":"class='info'")+"></span>");
		}
		$("#info-item7").html("<textarea class='form-control' id='"+7+"' style='width:400px;height:100px;resize:none;' readonly='readonly' value='"+arr[7]+"'/><span class='info1'></span>");
		...
		if (arr[8] != undefined) {
			...
			AjaxJson("../../handler/attachment/findAttachmentById", {
				...
				imgUrl = "../../" + data.attaLocation + data.attaFilename;
	...
	if("" != pic){
		...
		$.ajaxFileUpload({
			...
			'success' : function(data, status) {
				...
				AjaxJson("../../handler/attachment/findAttachmentById",{attaId:attachmentId},function(backData){
					var data = backData.data.attachment;
					if(data==null){
						$("#head-pic").attr("src","../../upload/portrait/user.jpg");
						alert("1111");
