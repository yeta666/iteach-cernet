...
function getNavibar(data) {
	...
	//暂时未做登录与未登录区分
	var headerHtml = "<li class=\"navbar-left\"><h1><a href=\"home.html\" title=\"首页\"></a>"+ systemName+ "</h1></li><li class=\"navbar-right\"><a href='#' onclick='loginOff();'><i class=\"fa fa-sign-out\" href='#' onclick='loginOff()'></i> 退出</a></li>"
...
function highlightColum() {
	...
	if (columnId > 0)
		$("#firstCol" + columnId + "").attr("class", "active"); // 添加菜单选中样式
	var columnId2 = getRequest("secondCol");
	if (columnId2 > 0) {
		$("#subCols").find('li').each(function() {
			$(this).removeClass();
		});
		$("#secondCol" + columnId2 + "").attr("class", "active"); // 添加菜单选中样式
	}
	return;
}
//Banners
