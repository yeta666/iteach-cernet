...
function getParameter() {
	...
	if ($("#veseOuterIp").val().replace(/\s/g,"") == '' || $("#veseOuterIp").val().replace(/\s/g,"") == undefined) {
		...
		parameter.veseOuterIp = $("#veseOuterIp").val().replace(/\s/g,"");
	if ($("#veseOuterMask").val().replace(/\s/g,"") == '' || $("#veseOuterMask").val().replace(/\s/g,"") == undefined||!reg.test($("#veseOuterMask").val().replace(/\s/g,""))) {
		infoNotice("error", "输入错误！", "客户端IP子网掩码 为空或格式错误！", $('#data-grid'));
		isOk = false;
		return false;
	} else
		parameter.veseOuterMask = $("#veseOuterMask").val().replace(/\s/g,"");
...
function addServer() {
	$("#addButton").click(function() {
		...
		if (isOk) {
			alert("isOK");
			var url = "../../handler/videoServer/insertServer";
			$.post(url, parameter, function(data) {
				if (data.ret) {
					var messgae = data.data.message;
					var state = data.data.state;
					infoNotice(state,"", messgae);
				}
			}, "json");
		}
	});
