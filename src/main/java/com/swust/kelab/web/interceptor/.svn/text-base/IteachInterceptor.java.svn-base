...
public class IteachInterceptor implements HandlerInterceptor {
     ...
     * @author yangzq
     * @param request
     * @param auVal 用户拥有的权限白名单
     * @param actVal 需要拦截的权限黑名单
     * @return
     */
    private boolean validateAuthority(String auVal, String actVal, HttpServletRequest request,
         ...
         * response.setStatus(CommonUtil.AUTHORITY_REFUSE); return false; } } if (actVal.contains(actionUrl)) { if
         * (!auVal.contains(actionUrl)) { response.setStatus(CommonUtil.AUTHORITY_REFUSE); return false; } }
         */
